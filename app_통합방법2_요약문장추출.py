...
def external_search_and_generate_response(request: Union[QueryRequest, str], session_id: str = None) -> dict:
    ...
    system_prompt = (
        f"""System:
            ...
            *사용자의 문장을 이해해서 추측도 반드시 해본다! 질문에 대해서 답을 생성도 같이해서 제일 앞에 단어를 붙인다.*
            ...
            [카테고리 검색용 상품 요약 문장 생성]
            사용자가 찾고자 하는 상품을 이해하고, 카테고리 검색에 최적화된 자연스러운 한국어 문장으로 요약하세요.
            
            **요구사항:**
            1. 사용자가 찾는 상품을 **완전한 문장 형태**로 설명 (키워드 나열 금지!)
            2. **자연스러운 문장 구조** 사용
            3. 카테고리 매칭에 도움이 되는 사람중심에서 관형오로 표현(먹는,쓰는,입는,바르는,때리는,던지는 등등) 포함
            4. 브랜드명보다는 **상품 유형과 특성**에 집중
            5. 길이: 10~20자 내외의 자연스러운 완전한 문장
            
            **올바른 예시:**
            - "여름용 작은 가방" → "여름에 사용할 수 있는 쿨소재의 여성용 작은 가방입니다"
            - "운동할 때 신는 신발" → "스포츠 활동에 적합한 편안한 운동화를 찾고 있습니다"  
            - "겨울 따뜻한 외투" → "겨울철 방한 기능이 뛰어난 보온 코트입니다"
    ...
    # final_results 초기화하고 방법2 먼저 추가
    final_results = []
    
    # 1. 방법2 결과 5개 먼저 추가
    if method2_all_sets:
        final_results.extend(method2_all_sets[0][:5])
        print(f"[최종표시] 방법2 결과 {len(method2_all_sets[0][:5])}개 추가됨")
    
    # 2. 방법1 결과 5개 나중에 추가
    if method1_all_sets:
        final_results.extend(method1_all_sets[0][:5])
        print(f"[최종표시] 방법1 결과 {len(method1_all_sets[0][:5])}개 추가됨")
    
    print(f"[최종표시] 총 표시 결과 수: {len(final_results)}개")
    ...
    **무조건 답변은 한글은 100자 이내,영어는 150자 이내로 자세하게 작성하되 요약본으로 답변을 작성하세요.
...
if __name__ == '__main__':
    ...
    uvicorn.run("app_통합방법2_요약문장추출:app", host=host, port=port, reload=debug)